...
const _UPDATE_ALL = function(options, cb) {
  try {
    ...
    Model.updateAsync(query, update)
...
const _CREATE = function(options, cb) {
  try {
    ...
    if (Array.isArray(newdoc) && newdoc.length && options.bulk_create) {
      ...
      Promisie.map(newdoc, Model.createAsync.bind(Model))
        .then(result => cb(null, result))
        .catch(cb);
    } else {
      Model.createAsync(utility.enforceXSSRules(newdoc, xss_whitelist, (options.newdoc) ? options : undefined))
        .then(result => cb(null, result))
        .catch(cb);
    }
  } catch (e) {
 ...
 * @type {Redshift_Adapter}
 */
const REDSHIFT_ADAPTER = class Redshift_Adapter {
  /**
   * Constructor for Redshift_Adapter
  ...
  constructor(options = {}) {
    ...
    this.adapter_type = 'redshift';

    if (options.db_connection && typeof options.db_connection === 'object') {
      if (options.db_connection instanceof Redshift) {
        ...
        this.db_connection = new Redshift(Object.assign({
          user: db_user,
    ...
    if (options.model && typeof options.model === 'object') {
      if (!(options.model instanceof Redshift.ORM)) {
        this.model = this.db_connection.import(options.model);
      } else {
...
module.exports = REDSHIFT_ADAPTER;